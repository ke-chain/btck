...
func createBlockChain(bc *ChainSPV) error {
...
func TestBlockchain_CommitHeader(t *testing.T) {
	bc, err := NewBlockchainSPV("", MockCreationTime, &chaincfg.RegressionNetParams)
	if err != nil {
		t.Error(err)
	}
...
func TestBlockchain_CheckHeader(t *testing.T) {
	...
	bc, err := NewBlockchainSPV("", MockCreationTime, params)
	if err != nil {
		t.Error(err)
	}
