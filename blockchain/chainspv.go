...
// ErrOrphanHeader represent Orphan block error
var ErrOrphanHeader = errors.New("header does not extend any known headers")

// ChainSPV Wrapper around Headers implementation that handles all blockchain operations
type ChainSPV struct {
	lock        *sync.Mutex
...
// NewBlockchainSPV returns a new chain with HeaderDB
func NewBlockchainSPV(filePath string, walletCreationDate time.Time, params *chaincfg.Params) (*ChainSPV, error) {
...
// GetEpoch returns a block header for calculate the difficulty.
func (b *ChainSPV) GetEpoch() (*wire.BlockHeader, error) {
...
func (b *ChainSPV) CheckHeader(header wire.BlockHeader, prevHeader StoredHeader) bool {
...
func (b *ChainSPV) GetCommonAncestor(bestHeader, prevBestHeader StoredHeader) (*StoredHeader, error) {
	var err error
...
func (b *ChainSPV) CommitHeader(header wire.BlockHeader) (bool, *StoredHeader, uint32, error) {
	b.lock.Lock()
	defer b.lock.Unlock()
	newTip := false
	...
	if header.PrevBlock.IsEqual(&tipHash) {
		...
		if err != nil {
			return false, nil, 0, ErrOrphanHeader
		}
	}
...
func (b *ChainSPV) GetNPrevBlockHashes(n int) []*chainhash.Hash {
...
func (b *ChainSPV) GetBlockLocator() BlockLocator {
	var ret []*chainhash.Hash
