...
func (ts *TxStore) GetKeyManager() *KeyManager {
	return ts.keyManager
}
...
func (ts *TxStore) GetPendingInv() (*wire.MsgInv, error) {
	...
	for _, utxo := range utxos {
		if utxo.AtHeight == 0 {
			txidMap[chainhash.Hash(utxo.Op.Hash)] = struct{}{} // adds to map
		}
	}
	// do the same with stxos based on height at which spent
	for _, stxo := range stxos {
		if stxo.SpendHeight == 0 {
			txidMap[chainhash.Hash(stxo.SpendTxid)] = struct{}{}
		}
	}
...
func (ts *TxStore) markAsDead(txid chainhash.Hash) error {
	...
	for _, s := range stxos {
		...
		spid := chainhash.Hash(s.SpendTxid)
		if txid.IsEqual(&spid) {
		...
		ophash := chainhash.Hash(s.Utxo.Op.Hash)
		if txid.IsEqual(&ophash) {
			if err := markStxoAsDead(s); err != nil {
				return err
			}
			if err := ts.markAsDead(spid); err != nil {
				return err
			}
		}
	}
	...
	for _, u := range utxos {
		uhash := chainhash.Hash(u.Op.Hash)
		if txid.IsEqual(&uhash) {
	...
	bid := btcdhash.Hash(txid)
	ts.Txns().UpdateHeight(bid, -1, time.Now())
	return nil
}
...
func (ts *TxStore) Ingest(tx *wire.MsgTx, height int32, timestamp time.Time) (uint32, error) {
	...
	utilTx := btcutil.NewTx(wire.Convert2btcd(tx)) // convert for validation
	...
	for i, txout := range tx.TxOut {
		...
		for _, script := range PKscripts {
			if bytes.Equal(txout.PkScript, script) { // new utxo found
				...
				newop := btcdwire.OutPoint{
					Hash:  btcdhash.Hash(cachedSha),
					Index: uint32(i),
				}
				...
				value += db.String2Int64(newu.Value)
				ts.Utxos().Put(newu)
				hits++
				break
			}
	...
	for _, txin := range tx.TxIn {
		for i, u := range utxos {
			newop := wire.OutPoint{
				Hash:  chainhash.Hash(u.Op.Hash),
				Index: u.Op.Index,
			}
			if outPointsEqual(txin.PreviousOutPoint, newop) {
				st := wallet.Stxo{
					Utxo:        u,
					SpendHeight: height,
					SpendTxid:   btcdhash.Hash(cachedSha),
				}
				ts.Stxos().Put(st)
				ts.Utxos().Delete(u)
				utxos = append(utxos[:i], utxos[i+1:]...)
				if !u.WatchOnly {
					value -= db.String2Int64(u.Value)
					hits++
				} else {
					matchesWatchOnly = true
				}
				...
				in := wallet.TransactionInput{
					...
					LinkedAddress: addr,
					Value:         *db.StringToBigInt(u.Value),
				}
	...
	if height > 0 {
		...
		for _, stxo := range stxos {
			bhash := btcdhash.Hash(cachedSha)
			if stxo.SpendTxid.IsEqual(&bhash) {
	...
	if hits > 0 || matchesWatchOnly {
		ts.cbMutex.Lock()
		ts.txidsMutex.Lock()
		txn, err := ts.Txns().Get(btcdhash.Hash(tx.TxHash()))
		...
		if err == nil && txn.Height <= 0 {
			ts.Txns().UpdateHeight(btcdhash.Hash(tx.TxHash()), int(height), txn.Timestamp)
			ts.txids[tx.TxHash().String()] = height
			if height > 0 {
				cb.Value = *db.StringToBigInt(txn.Value)
				shouldCallback = true
			}
		}
