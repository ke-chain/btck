...
func (s *StxoDB) Put(stxo wallet.Stxo) error {
	...
	_, err = stmt.Exec(outpoint, String2Int(stxo.Utxo.Value), int(stxo.Utxo.AtHeight), hex.EncodeToString(stxo.Utxo.ScriptPubkey), watchOnly, int(stxo.SpendHeight), stxo.SpendTxid.String())
	if err != nil {
		tx.Rollback()
		return err
	}
